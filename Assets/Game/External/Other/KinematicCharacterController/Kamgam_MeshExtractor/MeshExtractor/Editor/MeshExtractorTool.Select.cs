...
namespace Kamgam.MeshExtractor
{
    partial class MeshExtractorTool
    {
        ...
        protected void invertSelection()
        {
            ...
            TriangleCache.GetRenderers(_selectedObjects, meshRenderers, skinnedRenderers);
        ...
        protected void onSelectGUI(SceneView sceneView)
        {
            ...
            if ((evt.isMouse || evt.isScrollWheel || evt.isKey || evt.type == EventType.Repaint) && !evt.alt)
            {
                ...
                if(evt.isKey && evt.keyCode == MeshExtractorSettings.GetOrCreateSettings().TriggerSelectLinked && evt.type == EventType.KeyUp && !evt.control && !Tools.viewToolActive)
                {
        ...
        void addLinkedToSelection(bool remove = false)
        {
            ...
            var settings = MeshExtractorSettings.GetOrCreateSettings();
            var linkedTris = TriangleCache.AddLinked(_selectedObjects, _lastClickedTriangle, _limitLinkedSearchToSubMesh, settings.MaxSelectLinkedDuration, settings.ShowSelectLinkedFailedPopup);
